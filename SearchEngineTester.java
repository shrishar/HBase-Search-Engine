	...
	public static void main(String[] args) {
		...
		try {
			...
			if (command.equals("search-keyword")) {
				System.out.println("search-keyword " + args[1]);
...
//			System.out.println("pageDocId: " + pageDocId + " freq " + freq);

			
            // Step 2: get the URI of the page from clueWeb09DataTable
            byte[] uriBytes = Bytes.toBytes(pageDocId);
            Get gUriIndex = new Get(uriBytes);
            Result uriRow = dataTable.get(gUriIndex);
            pageUri = Bytes.toString(uriRow.getValue(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES));
            
//			System.out.println("pageUri: " + pageUri);

			
            // Step 3: get the page rank value of this page from clueWeb09PageRankTable
            byte[] pageRankBytes = Bytes.toBytes(pageDocId);
            Get gPRIndex = new Get(pageRankBytes);
            Result prRow = prTable.get(gPRIndex);
			pageRank = Bytes.toFloat(prRow.list().get(0).getQualifier());
...
//			System.out.println("pageRank: " + pageRank);
		...
		while (stack.size() > 0) {
			...
			System.out.println("word " + keyword + " NOT found");
		}
	}
